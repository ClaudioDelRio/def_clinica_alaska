 ...
 * Genera un PDF con las citas de un mes en particular
...
if (empty($medico_id) || empty($mes)) {
    die('Parámetros incompletos');
}

if (!preg_match('/^\d{4}-\d{2}$/', $mes)) {
    die('Formato de mes inválido');
}
...
try {
    ...
    if ($medico_id === 'todos') {
        ...
        ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$medico_id, $fechaInicio, $fechaFin]);
    }

    $citas = $stmt->fetchAll();

    setlocale(LC_TIME, 'es_ES.UTF-8', 'es_ES', 'Spanish_Spain', 'Spanish');
    $mesTexto = date('m-Y', strtotime($fechaInicio));

    $resumen = obtenerResumenEstadosMensual($citas);
    ...
    $html = generarHTMLReporteMensual(
        $nombreMedico,
        $especialidadMedico,
        $mesTexto,
        $fechaInicio,
    ...
    $nombreArchivo = 'Reporte_Mensual_' . str_replace(' ', '_', $nombreMedico) . '_' . $mes . '.pdf';
...
function generarHTMLReporteMensual($nombreMedico, $especialidadMedico, $mesTexto, $fechaInicio, $fechaFin, $citas, $totalCitas, $pendientes, $confirmadas, $completadas, $canceladas, $todosMedicos = false)
{
    $html = '
    <!DOCTYPE html>
    <html lang="es">
    ...
    $html .= '
        <div class="header">
            <h1>Informe Mensual de Citas</h1>
            <p>Clínica Veterinaria Alaska Pets Center</p>
        </div>';
    ...
    $html .= '
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">' . htmlspecialchars($mesTexto) . '</span>
            </div>
            <div class="info-row">
                <span class="info-label">Rango de fechas:</span>
                <span class="info-value">' . htmlspecialchars(date('d/m/Y', strtotime($fechaInicio)) . ' - ' . date('d/m/Y', strtotime($fechaFin))) . '</span>
            </div>
            <div class="info-row">
    ...
    if (empty($citas)) {
        $html .= '<div class="no-citas">No hay citas registradas en el mes seleccionado.</div>';
    } else {
...
function obtenerResumenEstadosMensual(array $citas): array
{
    $prioridad = [
        'cancelada' => 1,
        'pendiente' => 2,
